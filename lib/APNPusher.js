...
var onTransmissionError = function (errorCode, notification, recipient) {
    ...
    if (errorCode === 8 && recipient.token) {
        ...
        defaultRemoveDeviceHandler(token);
    }
};
...
var onFeedback = function (deviceInfos) {
    ...
    if (deviceInfos.length > 0) {
        _.each(deviceInfos.map(function (deviceInfo) {
            return deviceInfo.device.token.toString('hex');
        }), defaultRemoveDeviceHandler);
    }
};

var defaultRemoveDeviceHandler = function(token) {
    pushAssociations.removeDevice(token);
}

var removeDeviceHandler = function(handler) {
    defaultRemoveDeviceHandler = handler;
}
